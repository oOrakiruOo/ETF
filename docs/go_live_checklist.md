...
2. `reports/daily/notification_delivery_plan_YYYY-MM-DD.md` で High / Medium を確認
3. `data/processed/decisions/manual_decision_sheet_YYYY-MM-DD.csv` に判断・数量・価格・メモを記録
4. `reports/daily/go_live_readiness_YYYY-MM-DD.md` の判定を確認
5. `GO（手動確認後）` でも実売買は自動実行しない。MASATOが最終判断する
