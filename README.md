...
現在は「本運用準備の最終確認フェーズ」です。

- 完了: 日次判定、10年バックテスト、総当たり検証、局面別検証、弱点局面の採用監査
- 完了: 暫定本命ルールの固定、保有CSV評価、通知候補、手動判断CSV、週次PDCA
- 完了: 日次一括運用、タスクスケジューラ登録、携帯向け要約、LINE送信コマンド
- 進行中: LINE実設定後の疎通確認と本運用GO/HOLDの最終確認
- 次の実装候補: 実ポートフォリオCSV運用の定着、週次PDCAの実績蓄積、通知文の微調整

2026-06-18時点の暫定本命は `config/strategy_profiles.yaml` の `current_candidate` です。
...
最新の携帯向け要約、GO/HOLD、日次ヘルスの場所だけを確認する場合は、以下を使います。
...
LINEは送信専用です。自己確認ログが必要な場合は `self-check` を手動実行します。
